// src/modules/vivi_sdr/application/use-cases/disable-vivi-on-session.use-case.ts
...
interface DisableViviOnSessionInput {
  sessionId: string;
  tenantId: string;
}

@Injectable()
export class DisableViviOnSessionUseCase {
  constructor(
    @Inject('IWhatsAppSessionRepository')
    private readonly sessionRepository: IWhatsAppSessionRepository,
  ) {}

  async execute(input: DisableViviOnSessionInput): Promise<void> {
    ...
    await this.sessionRepository.updateAiEnabled(session.id, false);
  }
}
