// src/features/cadastro-publico/components/CorretorSignupForm.tsx
// Reaproveitado por /cadastro/corretor (tipoPerfil=corretor_house) e
// /cadastro/parceiro/corretor (tipoPerfil=corretor_parceiro) - mesmo
// formulario para os dois, so muda o tipoPerfil enviado e o titulo/link
// de volta.
"use client";
...
export function CorretorSignupForm({
  tipoPerfil,
  titulo,
  subtitulo,
  backHref,
}: CorretorSignupFormProps) {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-slate-50 px-4 py-10">
      <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
        ...
        <h1 className="mb-1 text-center text-xl font-semibold text-slate-800">{titulo}</h1>
        <p className="mb-6 text-center text-sm text-slate-500">{subtitulo}</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="mb-1 block text-sm text-slate-500">CRECI</label>
            <input
              type="text"
              required
              value={creci}
          ...
          <Link href={backHref} className="block text-center text-sm text-blue-600 hover:underline">
            Voltar
          </Link>
        </form>
      </div>
