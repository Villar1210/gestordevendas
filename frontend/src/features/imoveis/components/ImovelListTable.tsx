...
function formatValor(imovel: Imovel): string {
  ...
  if (imovel.rentPrice) parts.push(`${currencyFormatter.format(imovel.rentPrice)}/mês`);
...
export function ImovelListTable({ imoveis }: ImovelListTableProps) {
  ...
  return (
    <div className="overflow-x-auto rounded-2xl border border-slate-200 bg-white">
      ...
      {imoveis.length === 0 && (
        <p className="px-4 py-10 text-center text-sm text-slate-400">Nenhum imóvel encontrado.</p>
      )}
    </div>
  );
}
