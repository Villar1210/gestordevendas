      ...
      {cards.map((card) => (
        ...
        >
          ...
          {card.suggestedOwnerId ? (
            ...
            >
              {claimingId === card.id ? "Confirmando..." : "Confirmar Atribuição"}
            </button>
          ) : (
            <button
              onClick={() => handleStartAttendance(card)}
              disabled={claimingId === card.id}
              className="mt-2 w-full rounded-lg bg-blue-700 px-4 py-2 text-sm font-medium text-white hover:bg-blue-800 disabled:opacity-60"
            >
